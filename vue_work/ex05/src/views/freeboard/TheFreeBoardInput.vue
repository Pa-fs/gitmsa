<template>
    <div>
        ...
        class="w-full h-40 p-4 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent resize-none text-gray-700 placeholder-gray-400 bg-white"></textarea>
        </div>
    <button @click="save" 
...
const save = () => {
    ...
    .then(res => {
        console.log(res);
        alert('저장하였습니다.');
        router.push({path: "/freeboardSelect", params:{aa:10, bb:"안녕하세요"}});
    })
    .catch(e => {
        console.log(e);
